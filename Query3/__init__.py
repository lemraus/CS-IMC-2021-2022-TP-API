...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        with pyodbc.connect('DRIVER='+driver+';SERVER=tcp:'+server+';PORT=1433;DATABASE='+database+';UID='+username+';PWD='+ password) as conn:
            cursor = conn.cursor()
            cursor.execute("select count(*), birthYear from tArtist where birthYear = ( select top 1 birthYear from tArtist where birthYear != ‘0’ group by birthYear order by count(*) desc )")
