...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        with pyodbc.connect('DRIVER='+driver+';SERVER=tcp:'+server+';PORT=1433;DATABASE='+database+';UID='+username+';PWD='+ password) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT birthYear, COUNT(*) FROM [dbo].[tArtist] WHERE birthYear = ( SELECT TOP(1) birthYear FROM [dbo].[tArtist] WHERE birthYear != '0' GROUP BY birthYear ORDER BY COUNT(*) DESC ) GROUP BY birthYear")
